...
- v1.0.0 — Milestone: usable, demo-ready MVP, fully free-tier. *(IMPLEMENTED AND VERIFIED on `feature/v1.0.0-mvp-dashboard` @ 971a362 (mirrored on origin): evaluator dashboard UI, batch score-pending endpoint, typed frontend API client, backend+frontend test suites. Full suites GREEN as of the 2026-08-25 audit: pytest 334 passed (backend 104 + agents 230), Vitest 26/26 passed, ruff check+format clean. Migration 0007 applied; Node v24/npm present; .env present. REMAINING: version bump 0.7.0→1.0.0 release commit + CHANGELOG regen, optional eslint/prettier pass, live-credential E2E smoke of feedback/export endpoints, then merge to main + tag v1.0.0. See memory-bank/activeContext for exact state.)*
