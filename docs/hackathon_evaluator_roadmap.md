...
*Versioned milestones (v0.1.0 → v3.6.0) — 100% free-tier stack*
...
### v2.5.0 — Code quality analysis *(optional / stretch — deprioritized)*
...
**Build**
- For submissions that *do* include a repo link, use the free **GitHub REST API** (unauthenticated requests are free at low volume; add a free personal access token for higher limits) to pull README + file listing.
...
- Feed this as *additional* context into `TechnicalDepthAgent` — the agent must already work fully without it (per v0.5.0).
