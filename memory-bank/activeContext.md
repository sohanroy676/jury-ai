...
**v2.3.0 Explainability IMPLEMENTED (2026-09-01).** Citation support added to all four scoring agents — every score now includes a `cited_excerpt` field pointing to the specific submission content that drove the score. Design system overhaul complete (396-line CSS with design tokens, dark mode, responsive layout, app shell with header nav). Awaiting commit + next version. User confirmed: migrations applied, SMTP configured, real emails received — including the fixed explicit reject/shortlist wording. Full suites green on the release tree (pytest **392**, Vitest **45**). Branch stack `feature/v1.2.0-notifications` ← `feature/v1.2.0-batch-feedback` ← `fix/results-email-explicit-outcome` fast-forwards to `main`; tag `v1.2.0`; push approved by user this session. Delivered in v1.2.0: team_email collection (migration 0009 + portal field), confirmation + results emails over `EMAIL_PROVIDER=smtp|resend` (`services/email.py`, ADR-0004), one-click batch feedback (`POST /api/submissions/feedback-pending` + dashboard control), explicit shortlist/reject outcome copy in emails. Deadline reminder DEFERRED (needs roster/deadline model — see ROADMAP Next).
