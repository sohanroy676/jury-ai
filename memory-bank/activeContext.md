...
## Current focus
v0.3.5 (visual content understanding) is **implemented AND live-verified end to end** against a real 9-slide deck (`Clarix.pptx`, 21 content images / 16 unique after dedupe). Migrations 0004/0005 applied by user. Full pipeline verified through the real API: upload 201 -> text + 10 clean image descriptions stored -> structural filter excluded layout art and dropped ~6 high-confidence decorative images -> 9/10 low-confidence entries flagged `needs_human_review` -> cross-submission cache reuse proven (duplicate upload 23.8s vs 341.2s first run, ~14x; identical descriptions from cache; one previously-failed image retried successfully and cached) -> scoring 200 with specific justifications citing diagram-derived architecture details -> all 4 score rows persisted. One live bug found and fixed (commit `40e21e8`): qwen reasoning can exhaust the token budget leaving an UNCLOSED think block the stripper couldn't match — raised MAX_TOKENS to 2048 and strip truncated blocks to end-of-text (regression test added; 102 tests pass, ruff clean). Remaining before release: merge `feature/v0.3.5-image-understanding` to `main`, tag `v0.3.5`, push (user approval).
...
- Migrations 0004/0005 ARE applied (user, 2026-08-23). Live test data remains in Supabase: submissions `Clarix` (6e547440...) and `Clarix-Duplicate` (0631f0a9...), 9 image_cache rows, 4 scores rows — kept intentionally for inspection; safe to delete later.
- Known limitation (by design): one Clarix image (phash d44a2bb53ea236a5, slide 4 arrows graphic) persistently exhausts qwen's reasoning budget -> stored as `description: null` + `needs_human_review: true`. It is NOT cached, so future uploads retry it.

## Next step
- Merge `feature/v0.3.5-image-understanding` to `main`, tag `v0.3.5`, push (with explicit user approval). Then start the v0.4.0 checkpoint (core loop demoable end to end — now effectively already proven by this verification) in a fresh session per session-efficiency rules.
